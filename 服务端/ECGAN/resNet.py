...
class BasicBlock(nn.Module):
    # 表示输出通道数的倍乘，实际输出通道数为plane * expansion
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, dilation=1):
        super(BasicBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False, dilation=dilation)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False, dilation=dilation)
        self.bn2 = nn.BatchNorm2d(planes)

        self.shortcut = nn.Sequential()

        # strids不为1或者输入通道数不等于实际输出通道数时的操作
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=NUM_CLASSES):
        ...
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2, dilation=4)
        self.linear = nn.Linear(512 * block.expansion, num_classes)

    def _make_layer(self, block, planes, num_blocks, stride, dilation=1):
        strides = [stride] + [1]*(num_blocks-1)
        print("planes is {}, and strides is :{} ".format(planes, strides))
        layers = []
